...
\hldef{figG.data} \hlkwb{<-} \hlkwd{tibble}\hldef{(}\hlkwc{closer_vals} \hldef{= closer.data}\hlopt{$}\hldef{closer_vals,}
                    \hlkwc{farther_vals} \hldef{= farther.data}\hlopt{$}\hldef{farther_vals) |>}
  \hlkwd{mutate}\hldef{(}\hlkwc{val_diffs} \hldef{= closer_vals} \hlopt{-} \hldef{farther_vals)}
\hlcom{# create table for sweave}
\hlkwd{xtable}\hldef{(figG.data)}
\end{alltt}
\end{kframe}
\end{knitrout}
\textbf{Solution:} The code snippet above shows the code used for creating the table and generating the table \citep{table1} shown on the next page, using \texttt{xtable} \citep{xtable}.
\begin{table}[ht]
\centering
\begin{tabular}{rrrr}
  \hline
 & closer\_vals & farther\_vals & val\_diffs \\ 
  \hline
1 & 0.28 & -0.19 & 0.47 \\ 
  2 & 0.08 & -0.03 & 0.11 \\ 
  3 & 0.13 & -0.19 & 0.32 \\ 
